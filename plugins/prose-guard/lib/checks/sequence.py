...
def phases():
    try:
        ...
        notice.note(f"no checks could be read from {PHASE_DIR}, so this level ran only the "
                    f"deterministic checks")
        return []
    if not names:
        notice.note(f"there are no checks in {PHASE_DIR}, so this level ran only the "
                    f"deterministic checks")
